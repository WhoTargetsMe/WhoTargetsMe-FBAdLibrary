        ...
        for obj in details:
            ...
            if post_id is None:
                ...
                lower_bound_impressions, upper_bound_impressions = parse_bounds(
                    obj.get("impressions", None)
                )
                lower_bound_spend, upper_bound_spend = parse_bounds(
                    obj.get("spend", None)
                )
                ...
                ad_delivery_stop_time = cast_date(
                    obj.get("ad_delivery_stop_time", None)
                )

                if exists:
                    if (
                        finished_main_scripts()
                    ):  # if the script is called later in the day to get new ads
                        continue
                    ...
                    ad_delivery_start_time = cast_date(
                        obj.get("ad_delivery_start_time", None)
                    )
                    image_link = None
                    ad_info = None
                    ...
                    db.session.commit()
                    created_advert = (
                        db.session.query(Adverts).order_by(Adverts.id.desc()).first()
                    )
                ...
                created_impr = (
                    db.session.query(Impressions)
                    .order_by(Impressions.id.desc())
                    .first()
                )
                ...
                if region_distribution:
                    for it in region_distribution:
                        ...
                        reg = Region_distribution(impression_id, percentage, region)
