...
class Adverts(db.Model):
    ...
    ad_delivery_start_time = db.Column(db.DateTime(timezone=True), nullable=True, index=True)
    ad_delivery_stop_time = db.Column(db.DateTime(timezone=True), nullable=True)
    ad_snapshot_url = db.Column(db.Text, nullable=True) # temp link to ad in archive (includes token)
...
class Impressions(db.Model):
    ...
    __table_args__ = (UniqueConstraint('post_id', 'impressions', 'spend', 'region_distribution', 'demographic_distribution', name='ix_unique_post_id_impressions_spend_region_demographic'),)
...
class Demographic_distribution(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    impression_id = db.Column(db.Integer, db.ForeignKey('impressions.id'), nullable=False, index=True)
    ...
    __table_args__ = (UniqueConstraint('impression_id', 'age', 'gender', 'percentage', name='ix_unique_impression_id_age_gender_percentage'),)
...
class Region_distribution(db.Model):
    ...
    __table_args__ = (UniqueConstraint('impression_id', 'region', 'percentage', name='ix_unique_impression_id_region_percentage'),)
